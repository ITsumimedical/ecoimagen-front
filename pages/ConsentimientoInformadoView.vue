    ...
    <v-layout row justify-center>
        <v-flex xs12>
                    ...
                    <v-layout row wrap>
                        <table>
                            <tbody>
                                <tr>
                                    <td rowspan="3">
                                        <img src="images\sumimedicalLogo.jpeg" alt="" style="height: 130px;">
                                    </td>
                                    <td rowspan="3">
                    ...
                    <table>
                        <tbody>
                            <tr>
                                ...
                                <td style="height: 60px; text-align: center;">{{ datos.primer_apellido }}</td>
                                <td style="text-align: center;">{{ datos.segundo_apellido }}</td>
                                <td style="text-align: center;">{{ datos.primer_nombre }}</td>
                                <td style="text-align: center;">{{ datos.numero_documento }}</td>
            ...
            <p>Se me han aclarado todas las dudas y se me han dicho los beneficios, posibles riesgos y
                ...
                la Política de Tratamiento de Datos Personales disponible para su consulta en la página web <a href="https://sumimedical.com/politica-de-privacidad/" target="_blank">https://sumimedical.com/politica-de-privacidad/</a> la cual puede ser modificada
            ...
            <p v-if="datos.Edad_Cumplida < 19">En caso de que el paciente sea menor de 18 años, y/o incapaz, este
                consentimiento
                será suscrito por
                ...
                ciudadanía número: <b>{{ datos.docRepresentante }}</b> Otorgo mi Consentimiento en nombre del paciente
                menor de edad
                y/o
                incapaz, para que se le preste el servicio de Consulta Externa bajo la modalidad de Telemedicina
                Interactiva.
                SI_x_ NO___ (marque con una X).
            </p>

            <p v-else>En caso de que el paciente sea menor de 18 años, y/o incapaz, este consentimiento será
                suscrito por
                su representante legal.
                Sé que el paciente es incapaz de tomar por sí mismo la decisión de aceptar o rechazar el tratamiento
                descrito arriba. Por ende, en mi condición de Representante Legal del menor y/o incapaz
            ...
            <div class="firma mb-5" v-if="!firmado">
                ...
                <vueSignature width="100%" height="200px" :options="signatureOptions" ref="firmaPaciente" />
...
export default {
    ...
    name: 'WorkspaceJsonConsentimientoInformado',
    ...
    created() {
        this.consulta_id = this.$route.query.cita_id
    ...
    methods: {
        ...
        submit() {
            try {
                const firmaVacia = this.$refs.firmaPaciente.isEmpty();
                if (firmaVacia) {
                    return this.$swal({
                        title: "¡Falta información!",
                        text: "Debes Firmar El Documento",
                        type: "error",
                    });
                }
                const firma_paciente = this.$refs.firmaPaciente.save();
                let formdata = new FormData();
                formdata.append("firma", firma_paciente);
                formdata.append("consulta_id", parseInt(this.consulta_id));

                this.$axios.post('/auth/firmar-consentimiento?token=KMkm5PyrELKB2jnLMKyBgi8WgPwSNizSwwxJXBuY',formdata );
                this.consultar();
                this.limpiar();
            } catch (error) {
                console.log(error)
        ...
        async consultar() {
            try {
                ...
                const {
                    data
                } = await this.$axios.post('/auth/consultar?token=KMkm5PyrELKB2jnLMKyBgi8WgPwSNizSwwxJXBuY',formdata)
