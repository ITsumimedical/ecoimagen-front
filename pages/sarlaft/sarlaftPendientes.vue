        ...
        <v-dialog v-model="dialogo" persistent max-width="1400px">
            <v-card>
                ...
                <v-card-text>
                    <v-row dense>
                        ...
                        <v-col cols="12" sm="12" md="4">
                            <v-text-field v-model="informacion.tipo_cliente" readonly
                                label="¿Tipo cliente o proveedor?">
                            </v-text-field>
                        </v-col>
                        <v-col cols="12" sm="12" md="4">
                            <v-text-field v-model="informacion.nombre_diligencia" readonly
                                label="Nombre De Quien Diligencia">
                            </v-text-field>
                        </v-col>
                        <v-col cols="12" sm="12" md="4">
                            <v-text-field v-model="informacion.cedula_diligencia" readonly
                    ...
                    <v-row dense>
                        ...
                        <v-col cols="12" sm="12" md="4" v-if="informacion.tipo_cliente != 'Cliente Usuario Particular'">
                            <v-text-field v-model="representanteLegal.lugarExpedicion" readonly
                        ...
                        <v-col cols="12" sm="12" md="4">
                            <v-text-field v-model="representanteLegal.lugarNacimiento" readonly
                                label="Lugar De Nacimiento">
                            </v-text-field>
                        </v-col>
                        <v-col cols="12" sm="12" md="4">
                            <v-text-field v-model="representanteLegal.otra_nacionalidad" readonly
                        ...
                        <v-col cols="12" sm="12" md="4">
                            <v-text-field v-model="representanteLegal.ciudadResidencia" readonly
                        ...
                        <v-col cols="12" sm="12" md="4" v-if="informacion.tipo_cliente != 'Cliente Usuario Particular'">
                            <v-text-field v-model="representanteLegal.direccion_reci" readonly
                        ...
                        <v-col cols="12" sm="12" md="4">
                            <v-text-field v-model="representanteLegal.descripcion_obliga" readonly
                                label="Indique El Pais" required>
                            </v-text-field>
                        </v-col>
                    </v-row>
                    <v-row dense>
                    ...
                    <v-row dense>
                        ...
                        <v-col cols="12" sm="12" md="4">
                            <v-text-field v-model="informacionFinanciera.otros_ingresos" readonly
                                label="Otros ingresos">
                            </v-text-field>
                        </v-col>
                        <v-col cols="12" sm="12" md="4">
                            <v-text-field v-model="informacionFinanciera.concepto_ingreso" readonly
                        ...
                        <v-col cols="12" sm="12" md="4">
                            <v-text-field v-model="informacionFinanciera.concepto_egreso" readonly
                                label="Concepto de egresos">
                            </v-text-field>
                        </v-col>
                    </v-row>
                    <v-row dense>
                        ...
                        <v-col cols="12" sm="12" md="4">
                            <v-text-field v-model="operacionesInternacionales.otra_moneda" readonly
                                label="Indique otras">
                            </v-text-field>
                        </v-col>
                        <v-col cols="12" sm="12" md="4">
                    ...
                    <v-row dense>
                        ...
                        <v-col cols="12" sm="12" md="12">
                            <v-textarea v-model="revision.revision" outlined name="input-7-4"
                    ...
                    <v-row dense>
                        ...
                        <v-col cols="12" sm="12" md="2">
                            <v-btn color="success" @click="guardarRevision()" small>Guardar Revision</v-btn>
                        </v-col>
                    </v-row>
                    <v-row dense>
                        <v-col cols="12" sm="12" md="12" v-for="(revisionSarlaft, index) in revisionSarlaft"
                            ...
                            <v-alert dense text outlined color="primary" icon="mdi-order-bool-descending-variant"
                                class="text-center" v-if="revisionSarlaft">
                        ...
                        <v-col cols="12" sm="12" md="12" v-if="revisionSarlaft">
                            ...
                            <v-col cols="12" sm="12" md="12" xs12 v-for="(data, index) in revisionSarlaft.adjuntos"
                                :key="index">
                ...
                <v-card-actions>
                    ...
                    <v-btn small color="error" @click="terminarRevision()">
                        Terminar revisión
                    </v-btn>
                </v-card-actions>
            </v-card>
            ...
            <v-card-text>
                <v-row>
                    <v-tabs centered show-arrows>
                        <v-tab href="#tab-1" @click="tipo = 'Vinculacion Primera Vez', listarSarlaft()">
                            <v-icon left>mdi-card-account-details-outline</v-icon>
                            Vinculación primera vez
                        </v-tab>
                        <v-tab href="#tab-2" @click="tipo = 'Actualizacion Novedad', listarSarlaft()">
                            <v-icon left>mdi-card-account-details-outline</v-icon>Actualización novedad
                        </v-tab>
                        <v-tab href="#tab-1" @click="tipo = 'Actualizacion Anual', listarSarlaft()">
                            <v-icon left>mdi-card-account-details-outline</v-icon>Actualización anual
                        </v-tab>
                        <v-tab-item v-for="i in 3" :key="i" :value="'tab-' + i">
                            <v-card>
                                <v-card-text>
                                    <v-row>
                                        <v-col>
                                            <v-data-table dense :headers="headers" :items="listaSarlaft"
                                                disable-pagination hide-default-footer>
                                                <template v-slot:[`item.nombreSarlaft`]="{ item }">
                                                    <td v-if="item.user?.tipo_usuario_id === 2">
                                                        {{ obtenerNombreCompletoAfiliado(item.user?.afiliado) }}
                                                    </td>
                                                    <td v-else-if="item.user.tipo_usuario_id == 1"
                                                ...
                                                <template v-slot:[`item.acciones`]="{ item }">
                                                    <v-tooltip top>
                                                        <template v-slot:activator="{ on, attrs }">
                                                            <v-icon color="primary" class="mr-2" v-bind="attrs"
                                                                v-on="on" @click="informacionMostrar(item)">
                                                                mdi-eye
                                                            </v-icon>
                                                        </template>
                                            ...
                                            <v-row no-gutters>
                                                <v-col md="9" lg="9" xl="11" sm="11" class="px-2">
                                                    <v-pagination v-model="paginaOrden" class="my-4"
                                                        :length="totalOrden" :total-visible="9"
                                                        @input="listarSarlaft()">
                                                    </v-pagination>
                                                </v-col>
                                                <v-col md="3" lg="3" xl="1" sm="1" class="px-2" cols
                                                    style="min-width: 100px">
                                                    <v-select class="my-4" v-model="opcionActualOrden"
                                                        :items="opcionesOrden" dense solo small
...
export default {
    ...
    methods: {
        listarSarlaft() {
            this.preload = true
            this.$axios.post('sarlaft/listar?page=' + this.paginaOrden + '&cant=' + this
                .opcionActualOrden, {
                tipo: this.tipo
            }).then(res => {
        ...
        guardarRevision() {
            ...
            formData.append(`adjuntos`, this.$refs.adjuntoRevision.files[0]);
            formData.append(`revision`, this.revision.revision);
            formData.append(`sarlaft_id`, this.idSarlaft);
            this.$axios.$post('/revision-sarlaft/crear/' + this.sarlaft_id, formData).then(res => {
        ...
        terminarRevision() {
            this.preload = true
            this.$axios.post('sarlaft/finalizar', {
                idSarlaft: this.idSarlaft
            }).then(res => {
                this.dialogo = false
                this.listarSarlaft()
                this.preload = false
