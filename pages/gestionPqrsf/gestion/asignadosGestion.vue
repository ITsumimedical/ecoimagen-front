    ...
    <v-row dense class="mt-4">
      <v-col cols="12" md="2" sm="6">
        <v-text-field dense outlined append-icon="mdi-text-box-search-outline" label="N° Radicado"
          v-model="filtros.numero_radicado" @keyup.enter="listarPQRSFAsignados()"></v-text-field>
      </v-col>
      <v-col cols="12" md="2" sm="6">
        <v-text-field dense outlined append-icon="mdi-credit-card-search-outline" label="N° Documento"
          v-model="filtros.numero_documento" @keyup.enter="listarPQRSFAsignados()"></v-text-field>
      </v-col>
      <v-col cols="12" md="2" sm="6">
        <v-autocomplete dense outlined label="Departamento" item-text="nombre" item-value="id"
          v-model="filtros.departamento" :items="opcionesDepartamentos" :loading="loading.departamentos">
        </v-autocomplete>
      </v-col>
      <v-col cols="12" md="2" sm="6">
        <v-text-field dense outlined append-icon="mdi-barcode-scan" label="Código SuperSalud"
          v-model="filtros.codigo_super" @keyup.enter="listarPQRSFAsignados()"></v-text-field>
      </v-col>
      ...
      <v-col cols="12" md="1" sm="6">
        <v-menu dense v-model="menu.fechaInicio" :close-on-content-click="false" :nudge-right="40"
          ...
          <template v-slot:activator="{ on, attrs }">
            <v-text-field dense outlined label="Fecha Inicial" append-icon="mdi-calendar" v-bind="attrs" v-on="on"
              v-model="filtros.fecha_inicio"></v-text-field>
          </template>
      ...
      <v-col cols="12" md="1" sm="6">
        <v-menu dense v-model="menu.fechaFin" :close-on-content-click="false" :nudge-right="40"
          transition="scale-transition" offset-y min-width="auto">
          <template v-slot:activator="{ on, attrs }">
            <v-text-field dense outlined label="Fecha Final" append-icon="mdi-calendar" v-bind="attrs" v-on="on"
              v-model="filtros.fecha_fin"></v-text-field>
          </template>
          <v-date-picker v-model="filtros.fecha_fin" @input="menu.fechaFin = false"></v-date-picker>
        </v-menu>
      </v-col>
      <v-col cols="12" md="1" sm="6">
        <v-autocomplete dense outlined label="Canal" v-model="filtros.canal_id" :items="opcionesCanal"
          item-text="nombre" item-value="id" :loading="loading.canalesPQRSF"></v-autocomplete>
      </v-col>
      <v-col cols="12" md="1" sm="6">
        <v-autocomplete dense outlined label="Entidad" item-text="nombre" item-value="id" v-model="filtros.entidad_id"
          :items="opcionesEntidad" :loading="loading.entidades"></v-autocomplete>
      </v-col>
      <v-col cols="12" md="12" sm="12" align="right">
        <v-btn dark small color="warning" @click="limpiarFiltro" :loading="loading.PQRSFAsignados">Limpiar Filtros
        </v-btn>
        <v-btn dark small color="primary" @click="listarPQRSFAsignados()" :loading="loading.PQRSFAsignados">Buscar
        </v-btn>
      </v-col>
      ...
      <v-col cols="12" md="12" sm="12">
        <v-data-table disable-pagination hide-default-footer :items-per-page="paginacion.cantidadRegistros"
          :headers="headersPQRSFAsignados" :items="PQRSFAsignados" :loading="loading.PQRSFAsignados"
          loading-text="Cargando... Por favor espere" no-data-text="Sin datos para mostrar" dense>
          <template v-slot:[`item.entidadAfiliado`]="{ item }">
            <v-chip small :color="obtenerColorEntidad(item.afiliado.entidad.id)" dark>
          ...
          <template v-slot:[`item.asignadoPQRF`]="{ item }">
            <v-menu open-on-hover right offset-y>
              ...
              <v-list>
                <v-list-item v-for="area in item?.area_responsable" :key="area.id">
                  <v-list-item-content>
                    <v-list-item-title>{{ area?.nombre }}</v-list-item-title>
                    <v-list-item-subtitle>
                      <template v-if="area.responsable && area.responsable.length > 0">
                        <ul>
                          <li v-for="operador in area.responsable" :key="operador.id">
          ...
          <template v-slot:[`item.diasHabiles`]="{ item }">
            <v-chip small dark :color="obtenerColorDiasHabiles(item.priorizacion, item.diasHabiles)
              ">
          ...
          <template v-slot:[`item.acciones`]="{ item }">
            <v-tooltip bottom>
              <template v-slot:activator="{ on, attrs }">
                <v-icon color="primary" v-bind="attrs" v-on="on" @click="abrirModalAsignados(item)">mdi-eye</v-icon>
              </template>
              <span>Gestionar</span>
            </v-tooltip>
          </template>
      ...
      <v-col cols="12" md="11" sm="11">
        <v-pagination v-model="paginacion.pagina" :length="paginacion.total" :total-visible="9"
          @input="listarPQRSFAsignados()"></v-pagination>
      </v-col>
      <v-col cols="12" md="1" sm="1">
        <v-select dense v-model="paginacion.cantidadRegistros" :items="[5, 10, 20, 50, 100]" outlined
          label="Registros por Página" @change="listarPQRSFAsignados()"></v-select>
      </v-col>
    </v-row>

    <!-- Modales -->
    <modalPqrsfAsignado :mostrarModalAsignados.sync="mostrarModalAsignados" :PQRSFSeleccionada="PQRSFSeleccionada"
...
export default {
  ...
  methods: {
    listarPQRSFAsignados() {
      ...
      this.$axios
        .post("/formularios-pqrsf/listarAsignadasGestionArea", data)
        .then((res) => {
