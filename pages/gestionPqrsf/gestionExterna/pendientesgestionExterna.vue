<template>
  <div>
    <v-row dense class="mt-4">
      <v-col cols="12" md="2" sm="6">
        <v-text-field
          ...
          @keyup.enter="listarPQRSFPendientes()"
        ></v-text-field>
      </v-col>
      <v-col cols="12" md="2" sm="6">
        <v-text-field
      ...
      <v-col cols="12" md="12" sm="12" align="right">
        <v-btn
          ...
          color="warning"
          @click="limpiarFiltro"
          :loading="loading.PQRSFPendientes"
      ...
      <v-col cols="12" md="12" sm="12">
        <v-data-table
          ...
          :headers="headersPQRSFPendientes"
          :items="PQRSFPendientes"
          :loading="loading.PQRSFPendientes"
            ...
            <v-tooltip bottom>
              <template v-slot:activator="{ on, attrs }">
                <v-icon
                  ...
                  v-bind="attrs"
                  v-on="on"
                  @click="abrirModalPendientes(item)"
                  >mdi-eye</v-icon
                >
              </template>
              <span>Gestionar</span>
    ...
    <modalPqrsfPendiente
      ...
      :mostrarModalPendientes.sync="mostrarModalPendientes"
      @listarPQRSFPendientes="listarPQRSFPendientes"
    />
  </div>
</template>
<script>
import modalPqrsfPendiente from "~/components/pqrsf/centralPqrf/modalPqrsfPendiente.vue";
...
export default {
  ...
  methods: {
    listarPQRSFPendientes() {
      ...
      this.loading.PQRSFPendientes = true;
      this.$axios
        .post("/formularios-pqrsf/listarPendientesGestionExterna", data)
        .then((res) => {
          this.PQRSFPendientes = res.data.data;
        ...
        .catch(() => {
          this.$toast.error("Error al cargar los PQRF pendientes");
        })
        .finally(() => {
          this.loading.PQRSFPendientes = false;
        });
    },
    ...
    abrirModalPendientes(PQRSF) {
      this.mostrarModalPendientes = true;
      this.PQRSFSeleccionada = PQRSF;
    },
  },
};
</script>
