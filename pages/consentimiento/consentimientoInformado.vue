  ...
  <v-container>
    <v-layout row justify-center>
      <v-flex xs12>
        ...
        <v-btn color="primary" dark :block="true" @click="submit()" v-if="!firmado">Firmar</v-btn>
      </v-flex>
    </v-layout>
  </v-container>
</template>
...
layout:'consentimientos',
  ...
  created() {
    this.consulta = this.$route.query.cita_id
  ...
  methods: {
    ...
    async submit() {
      try {
        ...
        console.log(firma_paciente);
        if (firma_paciente.isEmpty) {
          this.$toast.warning('Se debe de llenar la firma')
          return false;
        }
        const request = {
          'firma': firma_paciente,
          'token': 'KMkm5PyrELKB2jnLMKyBgi8WgPwSNizSwwxJXBuY'
        }
        const response = await this.$axios.$post('cita/firmar-consentimiento/' + this.consulta +
    ...
    async consultar() {
      this.$axios.$get('cita/consultar/'+this.consulta+'?token=KMkm5PyrELKB2jnLMKyBgi8WgPwSNizSwwxJXBuY').then(res => {
          if (res.firma_consentimiento_time) {
            this.firmado = true;
          }
        this.datos = res
      }).catch(e => {
