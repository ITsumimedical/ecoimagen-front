<template>
    <div>
                ...
                <v-toolbar flat>
                    <v-toolbar-title>
                        <div class="d-flex align-center">
                            ...
                            <span>Autorización - Otros Servicios</span>
                        </div>
                    </v-toolbar-title>
                ...
                <v-row dense>
                    <v-col cols="12" sm="2" md="2">
                        ...
                        >
                            ...
                            <v-date-picker
                                v-model="filtros.fecha_inicial"
                                @input="menu.fecha_inicial = false"
                            ></v-date-picker>
                        </v-menu>
                    </v-col>
                    <v-col cols="12" sm="2" md="2">
                    ...
                    <v-col cols="12" sm="2" md="2">
                        <v-autocomplete dense outlined label="IPS" item-text="nombre" item-value="id"
                                        ...
                                        no-data-text="Escriba el Nombre o NIT de la IPS para buscar."
                                        :loading="loading.opcionesIps" :search-input.sync="campoBusquedaIps"/>
                    </v-col>
                    <v-col cols="12" sm="2" md="2">
                        <v-autocomplete dense outlined label="Departamento" item-text="nombre" item-value="id"
                                        ...
                                        :loading="loading.opcionesDepartamentos"/>
                    </v-col>
                    ...
                    <v-col cols="12" sm="12" md="12">
                        <v-data-table dense :headers="headersOrdenes" :items="ordenes"
                                      :loading="loading.ordenes"
                                      loading-text="Cargando... Por favor espere."
                                      no-data-text="Sin datos para mostrar."
                                      :items-per-page="paginacion.cantidadRegistros" disable-pagination
                                      hide-default-footer>
        ...
        <modal-gestionar-auditoria-codigos-propios
            :mostrar-modal-gestionar-auditoria.sync="mostrarModalGestionarAuditoria" :orden="ordenSeleccionada"
            @recargarDatos="listarOrdenes"/>
    </div>
</template>

<script>

import ModalGestionarAuditoriaCodigosPropios
    from "~/components/ordenamiento/codigosPropios/modalGestionarAuditoriaCodigosPropios.vue";

export default {
    components: {ModalGestionarAuditoriaCodigosPropios},
    middleware({$can, redirect}) {
        if (!$can('autorizacion.otrosServicios')) return redirect('/');
    },
    data() {
        return {
            loading: {
            ...
            mostrarModalGestionarAuditoria: false,
            ordenSeleccionada: {},
        }
    },
    mounted() {
    ...
    methods: {
        ...
        async listarOrdenes() {
            ...
            try {
                ...
                const response = await this.$axios.post('/auditoria/listar-ordenes-codigos-propios-por-auditar', data);
        ...
        abrirModalGestionar(orden) {
            ...
            this.mostrarModalGestionarAuditoria = true;
        }
    }
}
</script>
