<template>
    <div>
        ...
        <v-dialog v-model="dialogDetalle" fullscreen persistent>
            ...
            <v-card>
                ...
                <v-divider></v-divider>
                <v-alert border="left" text outlined dense type="info" icon="mdi-hospital-box-outline">
                    <h4>Servicios</h4>
                </v-alert>

                <v-data-table dense v-model="selectedProcedimientos" :headers="headersProcedimientoDetalle"
                    :items="servicios" :single-select="false" item-key="cup_id" show-select class="elevation-1"
                    hide-default-footer>
                </v-data-table>
                <v-col cols="12" sm="12" md="12">
                    <v-form>
                        ...
                        <v-row>
                            ...
                            <v-col cols="12" sm="12" md="3" v-if="selectedProcedimientos.length >= 1">
                                <v-checkbox v-model="posFechar" label="Pos-Fechar?" color="primary"></v-checkbox>

                            </v-col>
                            <v-col cols="12" sm="12" md="6"
                                v-if="selectedProcedimientos.length >= 1 && this.posFechar">
                ...
                <v-card-actions>
                    ...
                    <v-btn v-if="selectedArticulos.length > 0 || selectedProcedimientos.length > 0" small
                        color="success" @click="enviarEstado">Enviar</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
...
export default {
    ...
    methods: {
        ...
        buscarOrdenes() {
            this.$axios.$post('/auditoria/ordenamiento', this.formFilter).then(res => {
                this.ordenes = res;
            })
        },
        cargarDetalles(item) {
            ...
            this.$axios.$get('/auditoria/detalle-auditoria/' + item.consulta_id).then(res => {
                ...
                this.servicios = res.procedimientos;
        ...
        enviarEstado() {
            ...
            data.procedimientos = this.selectedProcedimientos;
