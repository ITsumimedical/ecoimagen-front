        ...
        <v-row class="mt-2">
            <v-col cols="12" sm="10" md="10">
                <v-text-field append-icon="mdi-magnify" dense label="Buscar"
                    hint="Ingrese el número de documento del afiliado" v-model="filtro.numeroDocumento" outlined>
                </v-text-field>
            </v-col>
            <v-col cols="12" sm="2" md="2">
                <v-btn color="primary" style="width: 49%" @click="listarNoProgramadasSinFinalizar()">Buscar</v-btn>
            ...
            <v-col cols="12" sm="12" md="12">
                <v-data-table :headers="headersNoProgramadasSinFinalizar" :items="noProgramadasSinFinalizar"
                    :loading="loading.noProgramadasSinFinalizar" loading-text="Cargando... Por favor espere" dense
                    ...
                    <template v-slot:[`item.afiliado_nombre`]="{ item }">
                        ...
                        item.afiliado?.segundo_apellido].filter(Boolean).join(' ') || 'Sin nombre' }}
                    </template>
...
export default {
    name: "noProgramadasSinFinalizar",
    components: {
        HistoriaIntegral,
    },
    ...
    data() {
        return {
            ...
            loading: {
                noProgramadasSinFinalizar: false,
            },
            noProgramadasSinFinalizar: [],
            headersNoProgramadasSinFinalizar: [
                { text: "N° Atención", value: "id" },
                { text: "Fecha Atención", value: "fechaAtencion" },
                { text: "Afiliado", value: "afiliado_nombre" },
    ...
    mounted() {
        this.listarNoProgramadasSinFinalizar();
    },
    methods: {
        ...
        listarNoProgramadasSinFinalizar() {
            this.loading.noProgramadasSinFinalizar = true;
            this.$axios
                .post("/consultas/noProgramadasSinFinalizar", this.filtro)
                .then((response) => {
                    this.noProgramadasSinFinalizar = response.data;
                })
                .catch((error) => {
                    this.$toast.error(
                        "Error al cargar las consultas sin finalizar, intente nuevamente"
                    );
                })
                .finally(() => {
                    this.loading.noProgramadasSinFinalizar = false;
                });
        },
