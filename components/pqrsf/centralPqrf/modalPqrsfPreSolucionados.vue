<template>
    <div>
        <v-dialog v-model="mostrarModalPreSolucionados" persistent max-width="2000px">
            <v-card>
                <v-alert text dense border="left" type="info" icon="mdi-checkbox-marked-circle-plus-outline"
                    class="text-center">
                ...
                ">
                            ...
                            <v-col cols="12" md="11" sm="11">
                                <v-autocomplete dense outlined label="Empleado(s)" :item-text="formatearNombre"
                                    ...
                                    :search-input.sync="campoBusquedaOperador" />
                            </v-col>
              ...
              <div>
                ...
                <v-card-actions>
                    ...
                    <v-btn :disabled="deshabilitarBoton" color="info" small dark
                        @click="abrirModalReAsignar">Reasignar</v-btn>
                    <v-btn color="warning" small dark @click="abrirModalAnular"
                        v-if="$can('pqrsf.anular')">Anular</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>

        <!-- Modales -->
        <modalReAsignarPqrsf :mostrarModalReAsignar.sync="mostrarModalReAsignar" :pqrsf="PQRSFSeleccionada"
            @cerrarModal="cerrarModal" @recargarPQRFs="listarPQRFPreSolucionados" />
...
import modalReAsignarPqrsf from "@/components/pqrsf/centralPqrf/modalReAsignarPqrsf.vue";
...
export default {
    ...
    watch: {
        mostrarModalPreSolucionados(newValue) {
            if (newValue) {
                this.buscarPQRSF();
            }
        },
    ...
    methods: {
        ...
        cerrarModal() {
            this.PQRSFActual = {};
            this.$emit("update:mostrarModalPreSolucionados", false);
        },
        ...
        listarPQRFPreSolucionados() {
            this.$emit("listarPQRFPreSolucionados");
        },
