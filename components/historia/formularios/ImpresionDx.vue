<template>
    <div>
        <v-form v-if="datos.cita.nombre !== 'Quimica Farmacologica'">
            <v-container grid-list-md fluid class="pa-0">
                <v-card-title class="headline" style="color:white;background-color:#0074a6">
                    <span class="title layout justify-center font-weight-light">Diagnosticos de la Consulta</span>
                </v-card-title>
                <v-layout row wrap>
                    <v-flex xs12 sm6 md6>
                        <v-autocomplete v-model="impresion.Cie10_id"
                            append-icon="mdi-magnify" :items="cie10s" item-disabled="customDisabled" item-text="nombre"
                            item-value="id" label="Diagnóstico">
                        </v-autocomplete>
                    </v-flex>
                    <v-flex xs12 sm6 md3>
                        <v-autocomplete small-chips :items="tipoDx" label="Tipo Diagnóstico"
