                    ...
                    <v-row dense>
                        <v-col cols="12" sm="6" md="3">
                            <v-select dense v-model="formAuditoria.estado" label="Seleccionar acción*"
                                :items="estadosActualizar" item-text="nombre" item-value="id"></v-select>
                        </v-col>
        ...
        <v-card>
            <v-card-text>
                <v-col cols="12" sm="12" md="12">
                    <v-row>
                        <v-col cols="12" sm="12" md="12">
                            <v-alert dense text border="left" color="info" class="text-center"><b>Histórico de ordenes
                                    de
                                    servicos</b>
                            </v-alert>
                        </v-col>
                        <v-col>
                            <v-text-field label="Buscar" hint="Buscar por cualquier campo" v-model="search" outlined
                                dense></v-text-field>
                        </v-col>
                        <v-col cols="12" sm="12" md="12">
                            <v-data-table dense :search="search" no-data-text="No hay registros"
                                v-if="ordenes.length > 0" :headers="headersServicios" :items="ordenes">
                                ...
                                <template v-slot:[`item.detalles`]="{ item }">
                                    ...
                                    <v-tooltip bottom
                                        v-if="(item.estado_id == 1 || item.estado_id == 4 || item.estado_id == 20)">
                                        <template v-slot:activator="{ on }">
                                            <v-btn icon color="blue-grey" v-on="on"
...
export default {
    name: 'WorkspaceJsonHistoricoServiciosComponent',
    components: {
        AfiliadoComponent,
    },
    props: {
