<template>
    ...
    <v-card elevation="1">
        <v-card-text>
            <v-form ref="filtrarOrden">
                <v-row dense>
                    ...
                    <v-col cols="12" sm="3" md="3">
                        <v-text-field label="Número documento" v-model="filtrarOrden.numero_documento" dense outlined
                    ...
                    <v-col cols="12" sm="3" md="3">
                        <v-autocomplete dense label="Usuario que ordenó" :items="operadores" item-value="user_id"
                            :item-text="item => `${item.nombre || ''} ${item.apellido || ''}`.trim()"
                            v-model="filtrarOrden.operadorNombre" outlined />
                    </v-col>
                        ...
                        <v-menu ref="menu" v-model="menu" :close-on-content-click="false" :return-value.sync="date"
                            ...
                            <template v-slot:activator="{ on, attrs }">
                                <v-text-field dense v-model="filtrarOrden.mes" type="month" outlined readonly
                                    label="Mes" append-icon="mdi-calendar" v-bind="attrs" v-on="on"></v-text-field>
                            </template>
                    ...
                    <td class="text-center">
                        <v-tooltip bottom>
                            <template v-slot:activator="{ on }">
                                <v-btn icon color="green" v-on="on" :disabled="generando" @click="imprimir(item)">
                        ...
                        <v-tooltip top>
                            <template v-slot:activator="{ on }">
                                <v-btn icon color="info" v-if="$can('autorizacionMipres.autorizarOrden')" v-on="on"
                                    :disabled="generando" @click="autorizar(item)">
        ...
        <v-dialog v-model="autorizarModal" max-width="800px" persistent>
            <v-card>
                <v-alert dense color="info" text icon="mdi-information">
                    Medicamento: {{ ordenSeleccionada?.id }}
                </v-alert>
                <v-card-text>
                    <v-form ref="formAutorizar">
                        <v-row dense>
                            <v-col cols="12" sm="12" md="12">
                                <v-textarea label="Observacion *" outlined v-model="formAutorizar.observaciones"
                                    :rules="[rules.obligatorio]"></v-textarea>
                            </v-col>
                        </v-row>
                    </v-form>
                </v-card-text>
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn small color="error" @click="cerrarModal"><v-icon>mdi-close</v-icon> Cerrar</v-btn>
                    <v-btn small color="success" @click="autorizarOrdenArticulo()"><v-icon>mdi-check</v-icon>
                        Autorizar</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
...
export default {
    ...
    methods: {
        ...
        autorizar(item) {
            ...
            this.autorizarModal = true;
        },

        async autorizarOrdenArticulo() {
            if (!this.$refs.formAutorizar.validate()) {
                this.$toast.error("¡ Por favor, Rellena el campo antes de Continuar !");
                return;
            }

            try {
                this.preload = true;
                await this.$axios.put(`/ordenamiento/autorizarOrdenArticulo/${this.ordenSeleccionada.id}`, { ...this.ordenSeleccionada, ...this.formAutorizar });
                this.$toast.success('Orden Autorizada Correctamente');
                this.cerrarModal();
                this.listar();
            } catch (error) {
                console.log(error);
        ...
        cerrarModal() {
            this.formAutorizar.observaciones = ''
