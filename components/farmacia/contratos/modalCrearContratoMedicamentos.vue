<template>
	<div>
		...
		<v-dialog v-model="mostrarModalCrearContrato" persistent max-width="900px" scrollable>
			<v-card>
				<v-alert dense text border="left" color="primary" icon="mdi-file-document-plus-outline"
						 class="text-center">
					<b>Agregar Contrato</b>
				</v-alert>
				<v-card-text>
					<v-form ref="formCrearContrato" class="mt-2">
						<v-row dense>
							<v-col cols="12" sm="12" md="6">
								<v-autocomplete dense outlined label="Entidad *" v-model="formContrato.entidad_id"
												:items="opcionesEntidades" item-text="nombre" item-value="id"
												:loading="loading.opcionesEntidades"
												:rules="[rules.obligatorio]"></v-autocomplete>
							</v-col>
							<v-col cols="12" sm="12" md="6">
								<v-autocomplete dense outlined label="Ámbito *" v-model="formContrato.ambito_id"
							...
							<v-col cols="12" sm="12" md="6">
								...
								>
									<template v-slot:activator="{ on, attrs }">
										<v-text-field
											v-model="formContrato.fecha_inicio"
							...
							<v-col cols="12" sm="12" md="6">
								...
								>
									<template v-slot:activator="{ on, attrs }">
										<v-text-field
											v-model="formContrato.fecha_vigencia"
											...
											v-on="on"
											outlined
											:rules="[rules.obligatorio,rules.fechaVigenciaValida]"
										></v-text-field>
									</template>
									<v-date-picker
										v-model="formContrato.fecha_vigencia"
							...
							<v-col cols="12" sm="12" md="12">
								<v-file-input dense outlined chips small-chips counter show-size label="Adjunto(s)"
											  ...
											  v-model="formContrato.adjuntos" multiple
				...
				<v-card-actions>
					<v-spacer></v-spacer>
					<v-btn color="error" small @click="cerrarModal">Salir</v-btn>
...
export default {
		...
		return {
			...
			rules: {
				...
				fechaVigenciaValida: v => {
					if (!this.formContrato.fecha_inicio) return 'Primero selecciona una fecha de inicio.';
					if (new Date(v) < new Date(this.formContrato.fecha_inicio)) {
	...
	watch: {
		mostrarModalCrearContrato(newValue) {
			if (newValue) {
				Promise.all([
		...
		'formContrato.fecha_inicio'(newValue) {
			this.$refs.formCrearContrato.validate();
		}
	},
	methods: {
		...
		cerrarModal() {
			...
			this.$emit('update:mostrarModalCrearContrato', false)
		},
		...
		limpiarFormulario() {
			...
			this.$refs.formCrearContrato.resetValidation();
		},

		async enviarFormulario() {
			if (!this.$refs.formCrearContrato.validate()) return;
			try {

				const formData = new FormData();

				this.formContrato.adjuntos = this.formContrato.adjuntos.map(adjunto => {
				...
				formData.append('prestador_id', this.prestadorId);
				formData.append('entidad_id', this.formContrato.entidad_id);
				formData.append('ambito_id', this.formContrato.ambito_id);
				formData.append('fecha_inicio', this.formContrato.fecha_inicio);
				formData.append('fecha_vigencia', this.formContrato.fecha_vigencia);

				// Agregar adjuntos con sus UUIDs al FormData
				this.formContrato.adjuntos.forEach((adjunto, index) => {
				...
				const response = await this.$axios.post('/contratos-medicamentos/crear-contrato', formData);
				...
				this.formContrato.adjuntos.forEach(adjunto => {
				...
				if (this.formContrato.adjuntos.length > 0) {
					// Subir los archivos
		...
		async subirArchivos() {
			try {
				...
				const adjuntos = this.formContrato.adjuntos;
		...
		async listarOpcionesEntidades() {
			try {
				...
				this.opcionesEntidades = response.data;
			} catch (error) {
