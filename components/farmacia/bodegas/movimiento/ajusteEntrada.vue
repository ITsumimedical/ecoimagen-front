  ...
  <div>
    ...
    <v-col cols="12" md="12">
      <v-row v-if="formSolicitudCompras.bodega_origen_id">
        <v-col cols="12" md="12">
          <v-btn color="success" :loading="preload" :disabled="preload" small rounded v-if="articulos.length > 0"
            @click="guardarAjuste()">
            Guardar ajuste
          </v-btn>
        </v-col>
        ...
        <v-col cols="12" md="12">
          <v-data-table v-model="articulos" :headers="articulosAjusteHeader" :items="listaSolicitudes"
            ...
            <template v-slot:[`item.cantidad_inicial`]="props">
              <v-edit-dialog :return-value.sync="props.item.cantidad_inicial" large persistent cancel-text="Cancelar"
                save-text="Cambiar">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      guardarAjuste() {
        this.preload = true
        this.$axios.post('solicitud-bodegas/guardarAjusteEntrada', {
