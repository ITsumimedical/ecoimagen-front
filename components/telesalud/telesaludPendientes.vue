    ...
    <v-row dense class="mt-4">
      ...
      <v-col cols="12" md="3" sm="3">
        <v-text-field
          ...
          @keyup.enter="listarTelesaludPendientes()"
          single-line
          clearable
          hide-details
          @click:clear="listarTelesaludPendientes()"
          append-icon="mdi-account-search"
        ></v-text-field>
      </v-col>
      <v-col cols="12" md="12" sm="12" align="right">
        <v-btn color="primary" @click="listarTelesaludPendientes()" small
          >Filtrar</v-btn
        >
      ...
      <v-col cols="12" md="12" sm="12">
        <v-data-table
          ...
          :items="telesaludPendientes"
          :loading="loading.telesaludPendientes"
      ...
      <v-col cols="12" md="11" sm="11">
        <v-pagination
          ...
          :total-visible="9"
          @input="listarTelesaludPendientes()"
        ></v-pagination>
      </v-col>
      <v-col cols="12" md="1" sm="1">
        <v-select
          ...
          @change="listarTelesaludPendientes()"
        ></v-select>
      </v-col>
    </v-row>

    <!-- Modal Telesalud Pendiente -->
    <modalTelesaludPendiente
      :telesaludSeleccionada="telesaludSeleccionada"
      :mostrarModalPendiente.sync="mostrarModalPendiente"
      @recargarTelesalud="listarTelesaludPendientes()"
    />
  </div>
</template>
<script>
import modalTelesaludPendiente from "@/components/telesalud/modalTelesaludPendiente.vue";
...
export default {
  ...
  mounted() {
    this.listarTelesaludPendientes();
  ...
  methods: {
    ...
    listarTelesaludPendientes() {
      const data = {
        filtros: this.filtros,
        paginacion: this.paginacion,
      };

      this.loading.telesaludPendientes = true;
      this.$axios
        .post("/telesalud/listarPendientes", data)
        .then((res) => {
          this.telesaludPendientes = res.data.data;
        ...
        .finally(() => {
          this.loading.telesaludPendientes = false;
        });
    },
