    ...
    <v-row dense class="mt-4">
      ...
      <v-col cols="12" md="3" sm="3">
        <v-text-field
          ...
          @keyup.enter="listarTelesaludJuntaSolucionadas()"
          single-line
          clearable
          hide-details
          @click:clear="listarTelesaludJuntaSolucionadas()"
          append-icon="mdi-account-search"
        ></v-text-field>
      </v-col>
      <v-col cols="12" md="12" sm="12" align="right">
        <v-btn color="primary" @click="listarTelesaludJuntaSolucionadas()" small
          >Filtrar</v-btn
        >
      ...
      <v-col cols="12" md="12" sm="12">
        <v-data-table
          ...
          hide-default-footer
          dense
          :headers="headersJuntaSolucionadas"
          :items="telesaludJuntaSolucionadas"
          :loading="loading.telesaludJuntaSolucionadas"
      ...
      <v-col cols="12" md="11" sm="11">
        <v-pagination
          ...
          :total-visible="9"
          @input="listarTelesaludJuntaSolucionadas()"
        ></v-pagination>
      </v-col>
      <v-col cols="12" md="1" sm="1">
        <v-select
          ...
          @change="listarTelesaludJuntaSolucionadas()"
        ></v-select>
      </v-col>
    </v-row>

    <!-- Modal Telesalud Junta Solucionada -->
    <modalTelesaludJuntaSolucionada
...
import modalTelesaludJuntaSolucionada from "@/components/telesalud/modalTelesaludJuntaSolucionada.vue";
...
export default {
  ...
  data() {
    return {
      ...
      loading: {
        telesaludJuntaSolucionadas: false,
        ips: false,
        tipoSolicitud: false,
      },
      menu: {
  ...
  mounted() {
    this.listarTelesaludJuntaSolucionadas();
  ...
  methods: {
    ...
    listarTelesaludJuntaSolucionadas() {
      const data = {
        filtros: this.filtros,
        paginacion: this.paginacion,
      };

      this.loading.telesaludJuntaSolucionadas = true;

      this.$axios
        .post("/telesalud/listarJuntaSolucionadas", data)
        .then((res) => {
          this.telesaludJuntaSolucionadas = res.data.data;
        ...
        .finally(() => {
          this.loading.telesaludJuntaSolucionadas = false;
        });
    },
