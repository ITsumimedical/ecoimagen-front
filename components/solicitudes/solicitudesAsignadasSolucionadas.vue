<template>
    <div>
      ...
      <v-row dense class="mx-4">
        ...
        <v-col cols="12" sm="12" md="12">
          <v-data-table disable-pagination hide-default-footer dense :headers="headersSolicitudesSolucionadas"
            :items="solicitudesSolucionadas.data" loading-text="Cargando..."
            :loading="loading.tablaSolicitudesSolucionadas">
            <template v-slot:[`item.estado`]="{ item }">
              <v-chip small :color="item.estado.id == 10 ? 'warning' : 'success'">
                {{ item.estado.id == 10 ? 'Pendiente' : 'Solucionado' }}
              </v-chip>
            </template>
  ...
  <script>
  import detallesAsignadasSolucionadas from '~/components/solicitudes/detallesAsignadasSolucionadas.vue';
  
    export default {
      name: "solicitudesAsignadasSolucionadas",
      components: {
        detallesAsignadasSolucionadas  
      },
      data() {
      ...
      created() {
        this.obtenerCantidadAsignadasSolucionadas();
      },
      methods: {
        obtenerCantidadAsignadasSolucionadas() {
          this.$axios
