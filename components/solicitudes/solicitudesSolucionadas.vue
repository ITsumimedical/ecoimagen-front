<template>
    <div>
      ...
      <v-row dense class="mt-4 mx-4">
        <v-col
          v-for="solicitud in tipoSolicitudes"
          :key="solicitud.id"
          cols="12"
          sm="2"
          md="2"
        >
          ...
          >
            <v-badge color="orange" :content="solicitud.quantity" bordered
              ><b>{{ solicitud.title }}</b></v-badge
            >
          </v-alert>
        </v-col>
      </v-row>
      ...
      <v-row dense class="mx-4">
        <v-col cols="12" sm="6" md="2">
          <v-text-field
            ...
            outlined
            label="N° Radicado"
            v-model="filtro.numero_radicado"
          ></v-text-field>
        </v-col>
        <v-col cols="12" sm="6" md="2">
          <v-text-field
            dense
            outlined
            label="N° Documento Afiliado"
            v-model="filtro.documento_afiliado"
          ></v-text-field>
        </v-col>
        ...
        >
          <v-btn color="info" dar style="width: 49%" @click="filtrarSolicitudes"
            >BUSCAR</v-btn
          >
          <v-btn color="error" dark style="width: 49%" @click="limpiarFiltros"
            >LIMPIAR</v-btn
          >
        </v-col>
        ...
        <v-col cols="12" sm="12" md="12">
          <v-data-table
            ...
            hide-default-footer
            dense
            :headers="headersSolicitudesSolucionadas"
            :items="solicitudesSolucionadas.data"
            loading-text="Cargando..."
            :loading="loading.tablaSolicitudesSolucionadas"
          >
          ...
          <v-pagination
            class="my-4"
            v-model="paginationSolicitudes.page"
            :length="
              Math.ceil(
            ...
            :total-visible="paginationSolicitudes.totalVisible"
          ></v-pagination>
        </v-col>
      </v-row>
  
      <!-- Modal Detalles -->
      <detallesSolucionadas
        :isOpen="abrirModalDetalles"
  ...
  <script>
  import detallesSolucionadas from "~/components/solicitudes/detallesSolucionadas.vue";
  export default {
    ...
    data() {
      return {
        ...
        headersSolicitudesSolucionadas: [
          {
            text: "Radicado",
            value: "id",
          },
        ...
        loading: {
          tablaSolicitudesSolucionadas: false,
        },
        filtro: {
    ...
    created() {
      this.obtenerCantidadSolucionadas();
    },
    methods: {
      obtenerCantidadSolucionadas() {
        this.$axios
          .get("/radicacion-online/cantidadSolucionadasTipo")
          .then((response) => {
          ...
          .catch((error) => {
            console.error("Error al obtener las cantidades solucionadas:", error);
          });
      },
      ...
      obtenerSolicitudes(tipo_id) {
        ...
        this.filtro.estado_id = 17;
        this.filtrarSolicitudes();
      },
  
      filtrarSolicitudes() {
        this.loading.tablaSolicitudesSolucionadas = true;
        this.filtro.page = this.paginationSolicitudes.page;
        this.filtro.per_page = this.paginationSolicitudes.rowsPerPage;
        this.$axios
          .post("/radicacion-online/listarSolicitudes", this.filtro)
          .then((res) => {
            this.solicitudesSolucionadas = res.data;
          ...
          .finally(() => {
            this.loading.tablaSolicitudesSolucionadas = false;
          });
      },
