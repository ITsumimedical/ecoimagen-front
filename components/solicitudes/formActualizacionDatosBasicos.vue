<template>
	<div>
		...
		<!-- Formulario de Actualización de Datos Basicos -->
		<v-form ref="formActualizacionDatosBasicos" @submit.prevent="enviarFormulario">
			<v-row dense>
        ...
        () => emailRegex.test(formCotizante.correo_cotizante) || 'Debe ser un correo electrónico válido'
				...
				<v-col cols="12" sm="12" md="12">
					<v-file-input dense label="Formato Integrado de Afiliación y Novedades (Fiduprevisora) *"
						v-model="formActualizacionDatos.adjuntoFormatoFiduprevisora" accept=".pdf, .doc, .docx" :rules="[
                () => !!formActualizacionDatos.adjuntoFormatoFiduprevisora || 'Este campo es requerido',
              ]">
					</v-file-input>
				</v-col>
				<v-col cols="12" sm="12" md="6">
					<v-file-input dense label="Documento de Identidad del Beneficiario *"
						v-model="formActualizacionDatos.adjuntoDocumentoBeneficiario" accept=".pdf, .doc, .docx" :rules="[
                () => !!formActualizacionDatos.adjuntoDocumentoBeneficiario || 'Este campo es requerido',
              ]"></v-file-input>
				</v-col>
				<v-col cols="12" sm="12" md="6">
					<v-file-input dense label="Documento de Identidad del Cotizante *"
						v-model="formActualizacionDatos.adjuntoDocumentoCotizante" accept=".pdf, .doc, .docx" :rules="[
                () => !!formActualizacionDatos.adjuntoDocumentoCotizante || 'Este campo es requerido',
              ]"></v-file-input>
				</v-col>
				<v-col cols="12" sm="12" md="12">
					<v-textarea dense auto-grow v-model="formActualizacionDatos.observaciones" label="Observaciones *"
						:rules="[
                () =>
                  !!formActualizacionDatos.observaciones || 'Este campo es requerido',
                () =>
                  formActualizacionDatos.observaciones.length >= 10 ||
	...
	export default {
		...
		methods: {
			...
			enviarFormulario() {
				...
				if (this.$refs.formActualizacionDatosBasicos.validate()) {
					...
					formData.append("descripcion", this.formActualizacionDatos.observaciones);
					...
					// Adjuntos como array
					const adjuntos = [
						this.formActualizacionDatos.adjuntoFormatoFiduprevisora,
						this.formActualizacionDatos.adjuntoDocumentoBeneficiario,
						this.formActualizacionDatos.adjuntoDocumentoCotizante,
