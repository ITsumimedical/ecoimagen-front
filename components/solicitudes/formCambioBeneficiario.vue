<template>
	<div>
		...
		<!-- Formulario de Cambio de Beneficiario Entre Cotizantes -->
		<v-form ref="formCambioBeneficiario" @submit.prevent="enviarFormulario">
			<v-row dense>
        ...
        () => emailRegex.test(formCotizante.correo_cotizante) || 'Debe ser un correo electrónico válido'
				...
				<v-col cols="12" sm="12" md="12">
					<v-file-input dense label="Formato Integrado de Afiliación y Novedades (Fiduprevisora) *"
						v-model="formCambio.adjuntoFormatoFiduprevisora" accept=".pdf, .doc, .docx" :rules="[
                () => !!formCambio.adjuntoFormatoFiduprevisora || 'Este campo es requerido',
              ]">
					</v-file-input>
				</v-col>
				<v-col cols="12" sm="12" md="6">
					<v-file-input dense label="Documento de Identidad del Beneficiario *"
						v-model="formCambio.adjuntoDocumentoBeneficiario" accept=".pdf, .doc, .docx" :rules="[
                () => !!formCambio.adjuntoDocumentoBeneficiario || 'Este campo es requerido',
              ]"></v-file-input>
				</v-col>
				<v-col cols="12" sm="12" md="6">
					<v-file-input dense label="Documento de Identidad del Cotizante *"
						v-model="formCambio.adjuntoDocumentoCotizante" accept=".pdf, .doc, .docx" :rules="[
                () => !!formCambio.adjuntoDocumentoCotizante || 'Este campo es requerido',
              ]"></v-file-input>
				</v-col>
				<v-col cols="12" sm="12" md="12">
					<v-textarea dense auto-grow v-model="formCambio.observaciones" label="Observaciones *" :rules="[
                () =>
                  !!formCambio.observaciones || 'Este campo es requerido',
                () =>
                  formCambio.observaciones.length >= 10 ||
	...
	export default {
		...
		methods: {
			...
			enviarFormulario() {
				...
				if (this.$refs.formCambioBeneficiario.validate()) {
					...
					formData.append("descripcion", this.formCambio.observaciones);
					...
					// Adjuntos como array
					const adjuntos = [
						this.formCambio.adjuntoFormatoFiduprevisora,
						this.formCambio.adjuntoDocumentoBeneficiario,
						this.formCambio.adjuntoDocumentoCotizante,
