<template>
	<div>
		...
		<!-- Formulario de Transcripción de Medicamentos -->
		<v-form ref="formTranscripcionMedicamentos" @submit.prevent="enviarFormulario">
			<v-row dense>
        ...
        () => emailRegex.test(formCotizante.correo_cotizante) || 'Debe ser un correo electrónico válido'
				...
				<v-col cols="12" sm="12" md="6">
					<v-file-input dense label="Historia Clínica *" v-model="formMedicamentos.adjuntoHC"
						accept=".pdf, .doc, .docx" :rules="[
                () => !!formMedicamentos.adjuntoHC || 'Este campo es requerido',
              ]">
					</v-file-input>
				</v-col>
				<v-col cols="12" sm="12" md="6">
					<v-file-input dense label="Orden de la Fórmula *" v-model="formMedicamentos.adjuntoOrden"
						accept=".pdf, .doc, .docx" :rules="[
                () =>
                  !!formMedicamentos.adjuntoOrden || 'Este campo es requerido',
              ]">
					</v-file-input>
				</v-col>
				<v-col cols="12" sm="12" md="12">
					<v-textarea dense auto-grow v-model="formMedicamentos.observaciones" label="Observaciones *" :rules="[
                () =>
                  !!formMedicamentos.observaciones || 'Este campo es requerido',
                () =>
                  formMedicamentos.observaciones.length >= 10 ||
	...
	export default {
		...
		methods: {
			...
			enviarFormulario() {
				...
				if (this.$refs.formTranscripcionMedicamentos.validate()) {
					...
					formData.append("descripcion", this.formMedicamentos.observaciones);
					...
					// Adjuntos como array,
					const adjuntos = [
						this.formMedicamentos.adjuntoHC,
						this.formMedicamentos.adjuntoOrden,
					];
