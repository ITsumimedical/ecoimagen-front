<template>
    <div>
      ...
      <!-- Modal Detalles -->
      <detallesPendientes :isOpen="abrirModalDetalles" :solicitud="solicitudSeleccionada" @close="cerrarModalDetalles"
        @enviarGestion="enviarGestion" @registrarBeneficiario="registrarBeneficiario" />
    </div>
  </template>
  
  <script>
    import detallesPendientes from "~/components/solicitudes/detallesPendientes.vue";
    ...
    export default {
      ...
      methods: {
        obtenerCantidadPendientes() {
          this.$axios
            .get("/radicacion-online/cantidadPendientesTipo")
            .then((response) => {
        ...
        enviarGestion(data) {
          ...
          switch (data.accion.id) {
            ...
            case 3:
              ...
              this.$axios
                .post("/radicacion-online/asignar", form)
                .then((res) => {
                  this.$toast.success(res.data);
                  this.obtenerCantidadPendientes();
                  this.obtenerSolicitudes(this.filtro.tipo_solicitud_id);
