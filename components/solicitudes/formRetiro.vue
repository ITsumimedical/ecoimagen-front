<template>
	<div>
		...
		<v-form ref="formRetiroAfiliado" @submit.prevent="enviarFormulario">
			<v-row dense>
        ...
        () => emailRegex.test(formCotizante.correo_cotizante) || 'Debe ser un correo electrónico válido'
				...
				<v-col cols="12" sm="12" md="12">
					<v-file-input dense label="Formato Integrado de Afiliación y Novedades (Fiduprevisora) *"
						v-model="formRetiro.adjuntoFormatoFiduprevisora" accept=".pdf, .doc, .docx" :rules="[
                () => !!formRetiro.adjuntoFormatoFiduprevisora || 'Este campo es requerido',
              ]">
					</v-file-input>
				</v-col>
				<v-col cols="12" sm="12" md="6">
					<v-file-input dense label="Documento de Identidad del Beneficiario *"
						v-model="formRetiro.adjuntoDocumentoBeneficiario" accept=".pdf, .doc, .docx" :rules="[
                () => !!formRetiro.adjuntoDocumentoBeneficiario || 'Este campo es requerido',
              ]"></v-file-input>
				</v-col>
				<v-col cols="12" sm="12" md="6">
					<v-file-input dense label="Documento de Identidad del Cotizante *"
						v-model="formRetiro.adjuntoDocumentoCotizante" accept=".pdf, .doc, .docx" :rules="[
                () => !!formRetiro.adjuntoDocumentoCotizante || 'Este campo es requerido',
              ]"></v-file-input>
				</v-col>
				<v-col cols="12" sm="12" md="12">
					<v-textarea dense auto-grow v-model="formRetiro.observaciones" label="Observaciones *" :rules="[
                () =>
                  !!formRetiro.observaciones || 'Este campo es requerido',
                () =>
                  formRetiro.observaciones.length >= 10 ||
	...
	export default {
		...
		methods: {
			...
			enviarFormulario() {
				...
				if (this.$refs.formRetiroAfiliado.validate()) {
					...
					formData.append("descripcion", this.formRetiro.observaciones);
					...
					// Adjuntos como array
					const adjuntos = [
						this.formRetiro.adjuntoFormatoFiduprevisora,
						this.formRetiro.adjuntoDocumentoBeneficiario,
						this.formRetiro.adjuntoDocumentoCotizante,
