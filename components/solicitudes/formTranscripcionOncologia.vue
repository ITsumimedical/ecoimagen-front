<template>
	<div>
		...
		<!-- Formulario de Transcripción Oncológica -->
		<v-form ref="formTranscripcionOncologia" @submit.prevent="enviarFormulario">
			<v-row dense>
        ...
        () => emailRegex.test(formCotizante.correo_cotizante) || 'Debe ser un correo electrónico válido'
				...
				<v-col cols="12" sm="12" md="6">
					<v-file-input dense label="Historia Clínica *" v-model="formOncologia.adjuntoHC"
						accept=".pdf, .doc, .docx" :rules="[
                () => !!formOncologia.adjuntoHC || 'Este campo es requerido',
              ]">
					</v-file-input>
				</v-col>
				<v-col cols="12" sm="12" md="6">
					<v-file-input dense small-chips label="Ordenes a Autorizar *" multiple
						v-model="formOncologia.ordenesAutorizacion" accept=".pdf, .doc, .docx" :rules="[
                () => !!formOncologia.ordenesAutorizacion.length || 'Este campo es requerido',
              ]">
					</v-file-input>
				</v-col>
				<v-col cols="12" sm="12" md="12">
					<v-textarea dense auto-grow v-model="formOncologia.observaciones" label="Observaciones *" :rules="[
                () =>
                  !!formOncologia.observaciones || 'Este campo es requerido',
                () =>
                  formOncologia.observaciones.length >= 10 ||
	...
	export default {
		...
		data() {
			return {
				...
				formOncologia: {
					observaciones: "",
					adjuntoHC: null,
					ordenesAutorizacion: [], // Agregado para múltiples archivos
				},
				ley1581: false,
		...
		methods: {
			...
			enviarFormulario() {
				...
				if (this.$refs.formTranscripcionOncologia.validate()) {
					...
					formData.append("descripcion", this.formOncologia.observaciones);
					...
					// Adjuntos como array
					formData.append("adjuntos", this.formOncologia.adjuntoHC);
					this.formOncologia.ordenesAutorizacion.forEach((archivo, index) => {
