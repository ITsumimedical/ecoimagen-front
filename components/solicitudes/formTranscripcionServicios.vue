<template>
	<div>
		...
		<v-form ref="formTranscripcionServicios" @submit.prevent="enviarFormulario">
			<v-row dense>
        ...
        () => emailRegex.test(formCotizante.correo_cotizante) || 'Debe ser un correo electrónico válido'
				...
				<v-col cols="12" sm="12" md="6">
					<v-file-input dense label="Soportes de Ayudas (Si Aplica) *" v-model="formServicios.adjuntoSoporte"
						accept=".pdf, .doc, .docx">
					</v-file-input>
				</v-col>
	...
	export default {
		...
		methods: {
			...
			enviarFormulario() {
				...
				if (this.$refs.formTranscripcionServicios.validate()) {
					...
					const adjuntos = [
						...
						this.formServicios.adjuntoSoporte,
