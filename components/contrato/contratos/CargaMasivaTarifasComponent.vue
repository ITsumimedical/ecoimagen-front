    ...
    <v-dialog v-model="dialog" width="600">
      ...
      <template v-slot:activator="{ on, attrs }">
        <v-btn small color="primary" v-bind="attrs" v-on="on">
          ...
          Cargue masivo a una sola tarifa
        </v-btn>
      </template>
  
      <v-card>
        <v-card-title>
          Cargue masivo de tarifas
        </v-card-title>
      ...
      methods: {
        ...
        async submit() {
          ...
          try {
            ...
            const response = await this.$axios.post('tarifa/carga-masiva/' + this.tarifa_id, request, {
              headers: {
                'Content-Type': 'multipart/form-data',
              },
            })
            this.$toast.success('Las tarifas se cargaron con exito');
            this.dialog = false
            this.$emit('cerrar');
          } catch (error) {
            this.$toast.error('Se encontraron algunos errores, Los CUPS que no tuvieron errores se cargaron con exito')
        ...
        plantilla(){
          this.setPreload(true);
          this.$axios.get("/tarifa/descargarPlantillaTarifaCup", {
              responseType: 'blob',
            }).then(res => {
              this.setPreload(false);
              ...
              a.download = 'PlantillaCargueTarifas.xlsx';
              a.href = url;
