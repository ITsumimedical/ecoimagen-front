<template>
    <div>
        <v-row dense class="mt-4">
            ...
            <v-col cols="12" md="2" sm="6">
                <v-autocomplete dense outlined :items="barreras" item-text="nombre" item-value="nombre" label="Tipo barrera" v-model="filtros.tipo_barrera" :loading="loadings.tipos" @focus="listarTiposBarrerasAcceso()"></v-autocomplete>
            </v-col>
            <v-col cols="12" md="2" sm="6">
            ...
            <v-col cols="12" md="1" sm="6">
                <v-menu dense v-model="menu.fechaInicio" :close-on-content-click="false" :nudge-right="40"
                    ...
                    <template v-slot:activator="{ on, attrs }">
                        <v-text-field dense outlined clearable label="Fecha Inicial" append-icon="mdi-calendar" v-bind="attrs"
                            v-on="on" v-model="filtros.fecha_inicio" @click:clear="filtros.fecha_inicio = null" readonly></v-text-field>
                    </template>
            ...
            <v-col cols="12" md="1" sm="6">
                <v-menu dense v-model="menu.fechaFin" :close-on-content-click="false" :nudge-right="40"
                    transition="scale-transition" offset-y min-width="auto">
                    <template v-slot:activator="{ on, attrs }">
                        <v-text-field dense outlined clearable label="Fecha Final" append-icon="mdi-calendar" v-bind="attrs"
                            v-on="on" v-model="filtros.fecha_fin" @click:clear="filtros.fecha_fin = null" readonly></v-text-field>
                    </template>
                    <v-date-picker v-model="filtros.fecha_fin" @input="menu.fechaFin = false"></v-date-picker>
                </v-menu>
            </v-col>
           <v-col cols="12" md="2" sm="6">
                <v-autocomplete dense outlined :items="reps" item-text="nombre" item-value="id" label="Centro de atención" v-model="filtros.sede" :loading="loadings.reps" no-data-text="Ingresa 4 letras para iniciar la búsqueda"
                  loading-text="Cargando... Por favor espere" :search-input.sync="buscarRep"></v-autocomplete>
            </v-col>
            <v-col cols="12" md="2" sm="6">
                <v-autocomplete dense outlined label="Asignado" v-model="filtros.asignadoA" :items="operadores" item-text="nombre" item-value="id" :loading="loadings.operador" :search-input.sync="buscarOperador" no-data-text="Ingresa 4 letras para iniciar la búsqueda"
            ...
            <v-col cols="12" md="12" sm="12" align="right">
                ...
                <v-btn small color="success" @click="listarSolucionadasAnuladas()">
            ...
            <v-col cols="12" md="12" sm="12">
                <v-data-table disable-pagination hide-default-footer loading-text="Cargando... Por favor espere"
                    :headers="headersSolucionados" :items="solucionasAnuladas" :loading="loading.solucionados"
                    no-data-text="Sin datos para mostrar" dense>
                    <template v-slot:[`item.observacion`]="{ item }">
                        <ObservacionComponente :observacion="item.observacion_solucion"></ObservacionComponente>
                    </template>
                    ...
                    <template v-slot:item.diasTranscurridos="{ item }">
                        <v-chip label dark small :color="item.color">
                            <b>{{ item.diasTranscurridos }} DÍA(S)</b>
                        </v-chip>
                    </template>
                    <template v-slot:item.estado="{ item }">
                        <v-chip label dark small :color="item?.estado_id == 17 ? 'green' : 'red'">
                            <b>{{ item?.estado_id == 17 ? 'SOLUCIONADO' : 'ANULADO' }}</b>
                        </v-chip>
                    </template>
        ...
        <ModalGestionarBarrearAcceso :mostrarModal.sync="mostrarModal" :barreraSelecionada="barreraSelecionada"
            @listar="listarSolucionadasAnuladas">
...
export default {
    ...
    mounted() {
        this.listarSolucionadasAnuladas();
    },
    ...
    methods: {
        listarSolucionadasAnuladas() {
            this.loading.solucionados = true;
            const data = {
            ...
            this.$axios.post(
                "/barrera-acceso/listar-barreras-solucionadas-anuladas?page=" + this.pagina + "&cant=" + this.opcionActual, data
            )
                .then((res) => {
                    this.solucionasAnuladas = res.data.data;
                    this.total = res.data.last_page;
                    this.$emit("actualizarConteo", res.data.total)
                })
                .catch((e) => {
                    this.$toast.error("Error al listar las barreras de acceso solucionadas y anuladas");
                })
                .finally(() => {
                    this.loading.solucionados = false;
                });
        },
