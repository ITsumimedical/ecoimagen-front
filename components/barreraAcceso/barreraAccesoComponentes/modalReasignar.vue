<template>
    <div>
        <v-dialog v-model="mostrarModalReasignar" max-width="600px" persistent>
            <v-card>
                <v-card-title>
                    <v-toolbar flat>
                        <v-toolbar-title>
                            <div class="d-flex align-center">
                                ...
                                <span>Reasignar N° {{ barreraSelecionada?.id }}</span>
                            </div>
                        </v-toolbar-title>
                    </v-toolbar>
                </v-card-title>
                            ...
                            <v-col cols="12" md="12" ms="12">
                                <v-col cols="12" md="12" sm="12">
                                    <v-autocomplete v-model="responsable_barrera_accesos_id" dense outlined multiple
                                        ...
                                        label="Nuevos responsables: *" deletable-chips :rules="rules.campoObligatorio">
                                    </v-autocomplete>
                                </v-col>
                            </v-col>
                        </v-row>
                ...
                <v-card-actions>
                    ...
                    <v-btn small color="success" @click="reasignarResponsable">REASIGNAR</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
    </div>
...
export default {
    ...
    watch: {
        mostrarModalReasignar(val) {
            if (val && this.barreraSelecionada && Array.isArray(this.barreraSelecionada.responsables)) {
                this.responsable_barrera_accesos_id = this.barreraSelecionada.responsables.map(r => r.id);
            }
        }
    },
    ...
    methods: {
        ...
        reasignarResponsable() {
            ...
            this.$axios.post('/barrera-acceso/reasignar-area-responsable', data)
                .then((res) => {
                    this.$emit('cerrarModalPadre');
                    this.cerrarModal();
                })
                .catch((error) => {
                    this.$toast.error("Hubo un error al asiganr los nuevos responsables")
                })
                .finally(() => {
