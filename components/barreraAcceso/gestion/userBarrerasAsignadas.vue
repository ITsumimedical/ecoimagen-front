    ...
    <div>
        <v-row dense class="mt-4">
            ...
            <v-col cols="12" md="12" sm="12" align="right">
                ...
                <v-btn small color="success" @click="listarBarrerasAsignadas()">
            ...
            <v-col cols="12" md="12" sm="12">
                <v-data-table disable-pagination hide-default-footer loading-text="Cargando... Por favor espere"
                    :headers="headersAsignadas" :items="asignadas" :loading="loadings.asignadas"
                    no-data-text="Sin datos para mostrar" dense>
                    <v-data-table :headers="headers" :items="items">
                        <template v-slot:[`item.user_crea`]="{ item }">
                            {{ [item.user_crea?.operador?.nombre,
                            item.user_crea?.operador?.apellido].filter(Boolean).join(' ') || 'Sin usuario' }}
                        </template>
                    </v-data-table>
        ...
        <ModalGestionarBarrearAcceso :mostrarModal.sync="mostrarModal" :barreraSelecionada="barreraSelecionada"
            @listar="listarBarrerasAsignadas">
...
export default {
    ...
    mounted() {
        this.listarBarrerasAsignadas()
    },


    methods: {
        listarBarrerasAsignadas() {
            this.loadings.asignadas = true;
            const data = {
            ...
            this.$axios.post('/barrera-acceso/listar-barreras-asignadas-user?page=' + this.pagina + "&cant=" + this.opcionActual, data)
                .then((res) => {
                    this.asignadas = res.data.data;
                    this.total = res.data.last_page;
                    this.$emit("actualizarConteo", res.data.total)
                })
                .catch((e) => {
                    this.$toast.error("Error al listar las barreras de acceso asignadas");
                })
                .finally(() => {
                    this.loadings.asignadas = false;
                });
        },
