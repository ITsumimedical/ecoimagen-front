<template>
    <v-card elevation="0">
            ...
            <v-row dense class="mt-4">
                <v-col cols="12" md="12" sm="12">
                    <v-data-table disable-pagination hide-default-footer loading-text="Cargando... Por favor espere"
                        :headers="headers" :items="responsables" :loading="loading"
                    ...
                    <v-row no-gutters>
                        <v-col md="9" lg="9" xl="11" sm="11" class="px-2">
                            <v-pagination v-model="pagina" class="my-4" :length="total" :total-visible="9" @input="listarResponsables">
                            </v-pagination>
                        </v-col>
                        <v-col md="3" lg="3" xl="1" sm="1" class="px-2" cols style="min-width: 100px">
                            <v-select class="my-4" v-model="opcionActual" :items="opciones" dense solo small @change="listarResponsables">
                            </v-select>
                        </v-col>
                    </v-row>
                </v-col>
        ...
        <ModalResponsable :mostrarModal.sync="mostrarModal" :modo="modo" :responsable="responsable"
            @listar="listarResponsables"></ModalResponsable>
    </v-card>
</template>
<script>
...
export default {
    ...
    methods: {
        ...
        listarResponsables() {
            this.loading = true;
            this.$axios.post(
                "/responsable-barrera-acceso/listar-responsables?page=" + this.pagina + "&cant=" + this.opcionActual
            )
                .then((res) => {
        ...
        async cambiarEstado(item) {
            try {
                const response = await this.$swal({
                    ...
                    text: "Al aceptar este mensaje, se cambiara el estado del responsable.",
                    type: "warning",
                ...
                if (response.value) {
                    ...
                    await this.$axios.put(`/responsable-barrera-acceso/cambiar_estado-responsable/${item.id}`)
                    await this.listarResponsables();
                }
            } catch (error) {
