<template>
    <v-card elevation="0">
            ...
            <v-row dense class="mt-4">
                <v-col cols="12" md="12" sm="12">
                    <v-data-table disable-pagination hide-default-footer loading-text="Cargando... Por favor espere"
                        :headers="headers" :items="tipos" :loading="loading" no-data-text="Sin datos para mostrar"
                        dense>
        ...
        <ModalTipoBarreraAcceso :mostrarModal.sync="mostrarModal" :modo="modo" :tipo="tipo"
            @listar="listarTiposBarrerasAcceso" />
    </v-card>
</template>
<script>
import { mapActions } from 'vuex'
import ModalTipoBarreraAcceso from './modalTipoBarreraAcceso.vue';


export default {
    components: {
        ModalTipoBarreraAcceso,
    },
    ...
    methods: {
        ...
        listarTiposBarrerasAcceso() {
            this.loading = true;
            this.$axios.post(
                "/tipo-barrera-acceso/listar-tipos-barrera_acceso?page=" + this.pagina + "&cant=" + this.opcionActual
            )
                .then((res) => {
        ...
        async cambiarEstado(item) {
            try {
                const response = await this.$swal({
                    ...
                    text: "Al aceptar este mensaje, se cambiara el estado del tipo de barrera acceso.",
                    type: "warning",
                ...
                if (response.value) {
                    ...
                    await this.$axios.put(`/tipo-barrera-acceso/cambiar_estado-tipo_barrera_acceso/${item.id}`)
                    await this.listarTiposBarrerasAcceso();
                }
            } catch (error) {
