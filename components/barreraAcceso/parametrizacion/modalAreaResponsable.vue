                ...
                <v-toolbar flat>
                    <v-toolbar-title>
                        <div class="d-flex align-center">
                            ...
                            <span class="ml-3">{{ esEditar ? 'Editar Area Responsable' : 'Crear Area Responsable' }}</span>
                        </div>
                    </v-toolbar-title>
                </v-toolbar>
            </v-card-title>
                ...
                <v-form ref="form" v-model="valid" lazy-validation>
                    <v-row dense>
                        <v-col cols="12" sm="12" md="12">
                            <v-text-field v-model="form.nombre" label="Nombre del Area: *" outlined dense
...
export default {
    ...
    watch: {
        dialog(val) {
            if (val) {
                if (this.esEditar && this.areaResponsable) {
                    this.form.nombre = this.areaResponsable.nombre;
                } else {
                    this.form = { nombre: "" };
                }
            }
    ...
    methods: {
        ...
        guardar() {
            ...
            const url = this.esEditar
                ? `/area-responsable-barrera-acceso/actualizar-area-responsable/${this.areaResponsable.id}`
                : `/area-responsable-barrera-acceso/crear-area-responsable`;
            ...
            this.$axios[metodo](url, this.form)
                ...
                .catch(error => {
                    this.$toast.error('Por favor, verifique el nombre del área. Ya existe un área registrada con ese mismo nombre.');
                })
                .finally(() => {
