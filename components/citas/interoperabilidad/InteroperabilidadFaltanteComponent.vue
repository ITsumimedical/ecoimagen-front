<template>
  <div>
    <v-data-table :headers="headersFaltantes" :items="logs" dense :page.sync="paginacion.current_page"
      :items-per-page="10" class="text-no-wrap" hide-default-footer :loading="loading.logs" @update:page="listarLogs">
      ...
      <template v-slot:top>
        <v-row>
          ...
          <v-col cols="12" md="3">
            <v-btn color="primary" :disabled="!logs.length || loading.logs" :loading="loading.envioMasivo" v-if="$can('citas.interoperabilidad.enviar_masivo_keiron')"
  ...
  methods: {
    ...
    async listarLogs(page = 1) {
      ...
      try {
        ...
        const response = await this.$axios.post(`citas/listar-faltantes-keiron`, data)
        this.logs = response.data.data;
        this.paginacion.total = response.data.last_page;
        
        this.$emit('total', response.data.total);
      } catch (error) {
    ...
    async enviarCitasMasivamente() {
      ...
      try {
        ...
        await this.$axios.get("logs-keiron/envio-masivo-consulta");
        this.$toast.success("Citas enviadas exitosamente");
        this.listarLogs();
      } catch (error) {
        console.error(error);
