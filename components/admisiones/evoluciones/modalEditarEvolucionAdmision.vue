<template>
    <v-dialog v-model="mostrarModalEditar" scrollable fullscreen persistent max-width="1200px"
                ...
                <v-toolbar flat>
                    <v-toolbar-title>
                        <div class="d-flex align-center">
                            ...
                            <span>Editar Evolución {{ `N° ${evolucionEditada?.id}` }}</span>
                        </div>
                    </v-toolbar-title>
                    <v-divider class="mx-4" inset vertical></v-divider>
                    <ordenamiento :consulta="this.evolucionSeleccionada?.consulta?.id" :habilitarBoton="true"
                        :dialog="false" />
                </v-toolbar>
            </v-card-title>
            <v-card-text>
                <v-form ref="formEditarEvolucion">
...
export default {
    ...
    watch: {
        'evolucionEditada.presion_sistolica': 'calcularTensionArterial',
        'evolucionEditada.presion_diastolica': 'calcularTensionArterial',
        'mostrarModalEditar'(nuevoValor) {
            if (nuevoValor && this.evolucionSeleccionada) {
                this.evolucionEditada = JSON.parse(JSON.stringify(this.evolucionSeleccionada));
    ...
    methods: {
        ...
        cerrarModal() {
            this.$refs.formEditarEvolucion.reset();
        ...
        calcularTensionArterial() {
            const ps = parseFloat(this.evolucionEditada.presion_sistolica);
            const pd = parseFloat(this.evolucionEditada.presion_diastolica);

            if (!isNaN(ps) && !isNaN(pd)) {
                const pam = ((ps + (2 * pd)) / 3).toFixed(1);
                this.evolucionEditada.tension_arterial = parseFloat(pam);
            }
        },

        async enviarFormulario() {
            if (!this.$refs.formEditarEvolucion.validate()) return;
            try {
                this.setPreload(true);

                await this.$axios.patch(`/evoluciones/actualizar/${this.evolucionEditada?.id}`, this.evolucionEditada);

                this.$toast.success('Evolución actualizada correctamente.')

                this.cerrarModal()

                this.$emit('recargarDatos')
            } catch (error) {
                this.$toast.error('Ocurrió un error al actualizar la evolución.')
            } finally {
        ...
        async guardarDiagnostico() {
            ...
            const payload = {
                consulta_id: this.evolucionEditada?.consulta_id ?? null,
        ...
        async listarHistoricoDiagnosticos() {
            try {                
                ...
                const { data } = await this.$axios.post('/historia/listarCie10Historico', {
                    consulta: this.evolucionEditada?.consulta_id ?? null,
                })
